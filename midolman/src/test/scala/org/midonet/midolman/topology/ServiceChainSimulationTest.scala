...
//                                prevPortId, x.getSrvPort, nextVlan)
    ...
    feature("Test redirect without vlans") {
        scenario("Test") {
            ...
            simulate(
                packetContextFor(packet(vm1br1mac, vm2br1mac), vm1br1.getId))
            simulate(
                packetContextFor(packet(vm2br1mac, vm1br1mac), vm2br1.getId))
            simulate(
                packetContextFor(packet(vm1br2mac, vm2br2mac), vm1br2.getId))
            simulate(
                packetContextFor(packet(vm2br2mac, vm1br2mac), vm2br2.getId))
            ...
            var service1 = Service.newBuilder()
                .setId(UUID.randomUUID).setMac(svc1mac).setPort(svc1.getId)
                .build
            updateService(store, CreateOp(service1)) should be(true)
            var service2 = Service.newBuilder()
                .setId(UUID.randomUUID).setMac(svc2mac).setPort(svc2.getId)
                .build
            ...
            var chain1 = ServiceChain.newBuilder()
                .setId(UUID.randomUUID).setName("chain1").setPort(vm1br1.getId)
                .build
            updateChain(store, CreateOp(chain1)) should be(true)
            var chain2 = ServiceChain.newBuilder()
                .setId(UUID.randomUUID).setName("chain2").setPort(vm1br2.getId)
                .build
