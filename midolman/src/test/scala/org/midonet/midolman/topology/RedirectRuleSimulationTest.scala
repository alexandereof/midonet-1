...
class RedirectRuleSimulationTest extends MidolmanSpec with TopologyBuilder {
    ...
    feature("Test redirect rules") {
        scenario("Packets ingressing P1-bridge1 egress P2-bridge2") {
            val host = createHost()
        ...
        scenario("VM1's traffic is redirected to SVC1 and SVC2") {
            val host = createHost()
        ...
        scenario("Test port with multiple redirection chains") {
            val host = createHost()
