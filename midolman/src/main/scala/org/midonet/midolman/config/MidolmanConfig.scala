...
class MidolmanConfig(_conf: Config, val schema: Config = ConfigFactory.empty()) extends TypeFailureFallback {
    ...
    val recircMnName = Try(getString(s"$PREFIX.midolman.datapath.recircInternalName")).getOrElse("recircVeth1")
    val recircHostName = Try(getString(s"$PREFIX.midolman.datapath.recircExternalName")).getOrElse("recircVeth0")
    val recircMnMac = Try(getString(s"$PREFIX.midolman.datapath.recircInternalMac")).getOrElse("02:00:11:00:11:02")
    val recircHostMac = Try(getString(s"$PREFIX.midolman.datapath.recircExternalMac")).getOrElse("02:00:11:00:11:01")
    def recircMnAddr = Try(getString(s"$PREFIX.midolman.datapath.peerAddr")).getOrElse("169.254.123.130")
    def recircHostCidr = Try(getString(s"$PREFIX.midolman.datapath.addr")).getOrElse("169.254.123.129/30")
