...
trait InAndOutFilters extends SimDevice {
    ...
    final protected val filterInIgnoreAdmin: (PacketContext, ActorSystem, SimStep) => Result =
    { filterIn(true) }

    private def filterIn(ignoreAdminDown: Boolean): (PacketContext, ActorSystem, SimStep) => Result =
        ...
        for (filter <- filters) {
            ...
            r.action match {
                ...
                case RuleResult.Action.REDIRECT =>
                    ...
                    return targetPort.continueOut(context, as)
                case a =>
